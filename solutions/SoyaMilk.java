...
public class Main {
	...
	public static void main(String[] args) throws IOException{
		...
		while(scan.hasNext()) {
			...
			double whole = area(points) * w;
			
			
//			points.add(new Point(0, 0));
			double tan =  Math.tan(Math.toRadians(degrees));
			
//			double h2 = .5 *  * l * w / (l*tan);
//			double triArea = ((cos * h) * l) / 2 * w;
			double ans = 0;
			if(tan * l > h) 
				ans = .5*Math.pow(h, 2) *l * w/(l*tan);
			else
				ans = l*w*((h)-(tan*l*.5));
//			System.out.println(triArea);
//			System.out.println(cos * h);
			System.out.printf("%.3f mL\n", ans);
//			double negCos = Math.cos(Math.toRadians(90-degrees));
//			Point p = new Point(-1 * h * negCos, h * cos); // tl
//			points.add(p);
//			p = new Point(l * cos, l * cos); // br
//			points.add(p);
//			p = new Point(l - (h * cos), h * cos); // tr
//			points.add(p);
